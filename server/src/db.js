...
/**
 * OAuthStateStore - Persistent storage for OAuth CSRF states.
 * Prevents state loss when server restarts during OAuth flow.
 */
class OAuthStateStore {
  constructor() {
    ...
    // Prepared statements
    this.stmtGet = db.prepare('SELECT * FROM oauth_states WHERE state = ?');
    this.stmtSet = db.prepare(
      'INSERT OR REPLACE INTO oauth_states (state, provider, app_url, expires_at) VALUES (?, ?, ?, ?)'
    );
    this.stmtDelete = db.prepare('DELETE FROM oauth_states WHERE state = ?');
    this.stmtCleanup = db.prepare('DELETE FROM oauth_states WHERE expires_at < ?');
    this.stmtSize = db.prepare('SELECT COUNT(*) as count FROM oauth_states');
    this.stmtOldest = db.prepare('SELECT state FROM oauth_states ORDER BY created_at ASC LIMIT 1');
  ...
  get(state) {
    const row = this.stmtGet.get(state);
    if (!row) return null;
  ...
  set(state, data) {
    this.stmtSet.run(state, data.provider, data.appUrl, data.expiresAt);
  }

  delete(state) {
    this.stmtDelete.run(state);
  }
  ...
  cleanup() {
    ...
    if (result.changes > 0) {
      console.log(`[OAuth] Cleaned up ${result.changes} expired state(s)`);
    }
  }

  get size() {
    return this.stmtSize.get().count;
  }

  deleteOldest() {
    const row = this.stmtOldest.get();
    if (row) {
      this.delete(row.state);
    }
  }
}

module.exports = { UserStore, OAuthStateStore, db };
