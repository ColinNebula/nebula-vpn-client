...
const { UserStore, OAuthStateStore } = require('../db');
...
// Persistent OAuth state store (replaces in-memory Map)
...
// Note: oauthStateStore is now defined at the top with UserStore
// It uses persistent SQLite storage to survive server restarts
...
router.get('/oauth/:provider/start', (req, res) => {
  ...
  // Limit state store size to prevent database bloat
  if (oauthStateStore.size >= 5000) {
    oauthStateStore.deleteOldest();
  }
