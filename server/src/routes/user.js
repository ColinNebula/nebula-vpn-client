...
module.exports = function createUserRouter(users) {
  ...
  router.get('/profile', authMiddleware, (req, res) => {
    ...
    // Auto-create development user if it doesn't exist (development mode only)
    if (!user && req.user.email === 'dev@localhost') {
      user = {
        email: 'dev@localhost',
        ...
        oauthProvider: null,
        oauthId: null,
        password: null // Development user has no password
      };
      users.set(req.user.email, user);
      logger.info(`Auto-created development user: ${req.user.email}`);
    }
