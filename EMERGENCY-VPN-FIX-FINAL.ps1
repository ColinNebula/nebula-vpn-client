# EMERGENCY VPN ROUTING FIX - FINAL VERSION
# ==========================================
...
if (-not $isAdmin) {
    ...
    Write-Host "5. .\EMERGENCY-VPN-FIX-FINAL.ps1" -ForegroundColor White
    Write-Host ""
...
foreach ($route in $brokenRoutes) {
    try {
        ...
        Write-Host "  WARN - Failed to remove: $route" -ForegroundColor Yellow 
    }
}

Write-Host "  INFO - Cleanup complete: $routesRemoved broken routes removed" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Write-Host "  WARN - Network refresh had issues (may be normal)" -ForegroundColor Yellow
}
...
if ($vpnAdapter) {
    ...
    Write-Host "  FAIL - VPN adapter not found or not running!" -ForegroundColor Red
...
if ($physicalAdapter) {
    ...
    Write-Host "  FAIL - Physical adapter not detected!" -ForegroundColor Red
}
...
if ($physicalAdapter) {
    try {
        ...
        Write-Host "  WARN - Gateway detection failed" -ForegroundColor Yellow
    }
}

if (-not $physicalGateway) {
    Write-Host "  WARN - Gateway auto-detect failed, using common defaults..." -ForegroundColor Yellow
    ...
    foreach ($gw in $commonGateways) {
        try {
            ...
            if ($ping) {
                $physicalGateway = $gw
                Write-Host "  PASS - Physical Gateway (detected): $physicalGateway" -ForegroundColor Green
                break
            }
        } catch {
...
if ($vpnAdapter -and $physicalGateway) {
    ...
    foreach ($route in $localRoutes) {
        try {
            ...
            Write-Host "  PASS - Local route: $($route.Dest)/$($route.Mask) -> $($route.Gateway)" -ForegroundColor Green
        } catch {
            Write-Host "  WARN - Local route may already exist: $($route.Dest)" -ForegroundColor Yellow
        }
    }
    ...
    foreach ($route in $vpnRoutes) {
        try {
            ...
            cmd /c $cmd 2>$null
            $routesAdded++
            Write-Host "  PASS - VPN route: $($route.Dest)/$($route.Mask) -> $($route.Gateway)" -ForegroundColor Green
        } catch {
            Write-Host "  WARN - Route may already exist: $($route.Dest)" -ForegroundColor Yellow
        }
    }
    
    Write-Host "  INFO - Applied $routesAdded VPN routes" -ForegroundColor Green
} else {
    Write-Host "  FAIL - Cannot apply routing - missing VPN adapter or gateway" -ForegroundColor Red
}
...
try {
    ...
    if ($ping) {
        Write-Host "  PASS - Ping test: SUCCESS" -ForegroundColor Green
    } else {
        Write-Host "  FAIL - Ping test: FAILED" -ForegroundColor Red
    }
} catch {
    Write-Host "  FAIL - Ping test: ERROR" -ForegroundColor Red
}
...
try {
    ...
    if ($externalIP -match "^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)") {
        ...
        Write-Host "  WARN - IP may still be ISP (but connectivity works)" -ForegroundColor Yellow
        Write-Host "    This might be expected depending on your VPN server configuration" -ForegroundColor Gray
    }
} catch {
    Write-Host "  FAIL - External IP test: FAILED - no internet access" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "  PASS - DNS resolution: SUCCESS" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "  WARN - Route analysis failed" -ForegroundColor Yellow
}
...
Write-Host "4. If still issues, run: .\WORKING-VPN-CLEANUP-FINAL.ps1" -ForegroundColor White
Write-Host ""
