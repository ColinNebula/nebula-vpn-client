...
try {
    ...
    if ($wgInterface) {
        Write-Host "   ✓ VPN Interface: $($wgInterface.Name)" -ForegroundColor Green
        ...
        Write-Host "   ✗ No VPN interface found - VPN not connected" -ForegroundColor Red
        $allGood = $false
    }
} catch {
    Write-Host "   ⚠ Could not check VPN interface" -ForegroundColor Yellow
    $allGood = $false
}
...
try {
    ...
    if ($routes -match "0\.0\.0\.0\s+128\.0\.0\.0") {
        ...
        Write-Host "   ✓ Good: No aggressive route 0.0.0.0/1" -ForegroundColor Green
    }
    ...
    if ($routes -match "128\.0\.0\.0\s+128\.0\.0\.0") {
        ...
        Write-Host "   ✓ Good: No aggressive route 128.0.0.0/1" -ForegroundColor Green
    }
} catch {
...
foreach ($dns in $dnsServers) {
    if ($routes -match [regex]::Escape($dns)) {
        Write-Host "   ✓ Found DNS route: $dns" -ForegroundColor Green
        $dnsRoutesFound++
    }
}

if ($dnsRoutesFound -gt 0) {
    Write-Host "   ✓ Resilient routing active: $dnsRoutesFound DNS routes found" -ForegroundColor Green
} else {
    Write-Host "   ⚠ No DNS routes found - VPN may not be using resilient config" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "   ✓ Internet: WORKING (Public IP: $ip)" -ForegroundColor Green
} catch {
...
try {
    ...
    if ($gateway) {
        ...
        if ($ping) {
            Write-Host "   ✓ Local Network: WORKING (Gateway: $gateway)" -ForegroundColor Green
        } else {
            Write-Host "   ✗ Local Network: Cannot reach gateway $gateway" -ForegroundColor Red
            $allGood = $false
        }
    }
} catch {
...
try {
    ...
    Write-Host "   ⚠ DNS Resolution: Could not test" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "   ⚠ Could not check DNS configuration" -ForegroundColor Yellow
}
...
if ($allGood -and $wgInterface) {
    ...
    Write-Host "ℹ️  INFO: VPN not currently connected" -ForegroundColor Yellow
    Write-Host "   Connect your Nebula VPN and run this script again" -ForegroundColor Yellow
} elseif (!$allGood) {
    Write-Host "⚠️  WARNING: Issues detected with routing configuration" -ForegroundColor Red
