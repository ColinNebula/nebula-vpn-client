# EMERGENCY VPN ROUTING FIX - FIXED VERSION
# ==========================================
...
if (-not $isAdmin) {
    ...
    Write-Host "5. .\EMERGENCY-VPN-FIX-CORRECTED.ps1" -ForegroundColor White
    Write-Host ""
...
foreach ($route in $brokenRoutes) {
    try {
        ...
        Write-Host "  ⚠️ Failed to remove: $route" -ForegroundColor Yellow 
    }
}
...
if ($physicalAdapter) {
    try {
        ...
        Write-Host "  ⚠️ Gateway detection failed" -ForegroundColor Yellow
    }
}
...
if ($vpnAdapter -and $physicalGateway) {
    ...
    foreach ($route in $localRoutes) {
        try {
            ...
            Write-Host "  ⚠️ Local route may already exist: $($route.Dest)" -ForegroundColor Yellow
        }
    }
...
Write-Host "4. If still issues, run: .\WORKING-VPN-CLEANUP.ps1" -ForegroundColor White
Write-Host ""
