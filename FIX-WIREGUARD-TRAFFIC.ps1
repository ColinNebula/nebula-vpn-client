...
if (-not $vpnAdapter) {
    Write-Err "WireGuard adapter not found!"
...
if (-not $allowInterfaceRule) {
    Write-Err "MISSING: Firewall rule to allow traffic through VPN interface!"
...
if ($defaultRoute) {
    ...
    Write-Err "No default route found"
    $needsRoutingFix = $true
}
...
if ($vpnAdapter.Status -eq "Up") {
    ...
    Write-Err "VPN interface is $($vpnAdapter.Status)"
...
if ($vpnIP) {
    ...
    Write-Err "No IP address assigned to VPN interface"
}
    ...
    if ($confirm -eq "yes") {
        ...
        foreach ($rule in $nebulaRules) {
            try {
                ...
                Write-Err "Failed to remove $($rule.Name): $($_.Exception.Message)"
            }
        }
...
if ($FixAll -or $issuesFound.Count -gt 0) {
    ...
    if ($needsInterfaceFix) {
        ...
        try {
            ...
            Write-Err "Failed to create interface rule: $($_.Exception.Message)"
        }
    }
    ...
    if ($needsRoutingFix) {
        ...
        if ($confirm -eq "yes") {
            try {
                ...
                Write-Err "Failed to fix routing: $($_.Exception.Message)"
            }
        } else {
