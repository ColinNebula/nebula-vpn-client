...
Write-Host "â-‘                    Nebula VPN Launcher                         â-‘" -ForegroundColor Cyan
Write-Host "â-‘                                                                    â-‘" -ForegroundColor Cyan
...
if (-not (Test-AdminPrivileges)) {
    ...
    Write-Host "  â€¢ DNS enforcement (netsh commands)" -ForegroundColor Yellow
    Write-Host "  â€¢ VPN tunnel management (WireGuard)" -ForegroundColor Yellow
    Write-Host "  â€¢ Kill switch configuration" -ForegroundColor Yellow
    Write-Host ""
...
if (-not (Test-ServerRunning)) {
    ...
    $serverCmd = "cd '$serverPath'; Write-Host 'ðŸš€ Nebula VPN API Server' -ForegroundColor Green; Write-Host 'â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-â-' -ForegroundColor Green; Write-Host ''; node src\index.js"
    ...
    if (-not (Wait-ForServer -TimeoutSeconds $MaxServerWaitSeconds)) {
        ...
        Write-Host "  â€¢ Missing or invalid .env file" -ForegroundColor Yellow
        Write-Host "  â€¢ Port $ServerPort already in use" -ForegroundColor Yellow
        Write-Host "  â€¢ Missing dependencies (run 'npm install' in server folder)" -ForegroundColor Yellow
        Write-Host ""
        Read-Host "Press Enter to exit"
        exit 1
    }
...
Write-Host "â-‘               Nebula VPN Started Successfully!                 â-‘" -ForegroundColor Green
Write-Host "â-‘                                                                    â-‘" -ForegroundColor Green
