...
### "VPN Blocks WiFi/Local Network Access"
**Cause:** VPN routing all traffic including local network

**Symptoms:**
- Can't access router admin page
- Network printer not working
- Can't see other computers on network
- Local file shares not accessible
...
2. **Apply split tunnel fix:**
   ```powershell
   .\FIX-VPN-ROUTES-NOW.ps1
   ```

3. **Reconnect VPN for permanent fix**

**What it does:** Configures split tunneling so internet traffic goes through VPN while local network traffic (192.168.x.x, 10.x.x.x, 172.16-31.x.x) bypasses VPN and uses your WiFi/LAN adapter.
