...
function Show-DeploymentReport {
    ...
    Write-Host "Frontend Built:   $(if (Test-Path 'build\index.html') { '[OK]' } else { '[FAIL]' })" -ForegroundColor White
    Write-Host "Electron Built:   $(if (-not $SkipElectron -and (Test-Path 'dist')) { '[OK]' } else { '[SKIP]' })" -ForegroundColor White
    Write-Host "Backend Deployed: $(if ($DeployBackend) { '[OK]' } else { '[SKIP]' })" -ForegroundColor White
    Write-Host "Server Restarted: $(if ($RestartServer) { '[OK]' } else { '[SKIP]' })" -ForegroundColor White
    
    if ($Global:DeploymentErrors.Count -gt 0) {
        Write-Host "`nErrors encountered:" -ForegroundColor Red
        $Global:DeploymentErrors | ForEach-Object { Write-Host "  - $_" -ForegroundColor Red }
    } else {
        Write-Host "`n[OK] No errors!" -ForegroundColor Green
    }
