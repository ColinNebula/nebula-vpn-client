...
// Enhanced detection for packaged Electron apps.
const isElectronRuntime = typeof window !== 'undefined' && !!(window.electron || window.nebulaVPN);

// Enhanced development mode detection for packaged apps
const isLocalDevRuntime = 
  process.env.NODE_ENV === 'development' ||
  (typeof window !== 'undefined' && /localhost:3000|127\.0\.0\.1:3000/.test(window.location?.origin || '')) ||
  // Packaged Electron app development mode detection
  (isElectronRuntime && (
    process.env.FORCE_DEV_MODE === 'true' ||
    (typeof window !== 'undefined' && window.location?.protocol === 'file:' && process.env.NODE_ENV === 'development')
  ));
...
console.log('🔗 API_BASE_URL:', API_BASE_URL, 'isElectronRuntime:', isElectronRuntime, 'isLocalDevRuntime:', isLocalDevRuntime);
...
class APIService {
  ...
  async handleResponse(response) {
    ...
    if (!response.ok) {
      ...
      /** @type {any} */ (err).status = response.status; // preserve HTTP status so callers can distinguish auth vs network errors
      throw err;
    }
