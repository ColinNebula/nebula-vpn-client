...
const ACTION_TYPES = [
  ...
  { value: 'disable_killswitch', label: '[Unlocked] Disable Kill Switch',    hasServer: false },
  { value: 'enable_split',       label: '[Cut] Enable Split Tunneling',  hasServer: false },
  { value: 'disable_split',      label: '🔗 Disable Split Tunneling', hasServer: false },
  { value: 'notify',             label: '[Bell] Send Notification',      hasServer: false },
];
...
const Toast = ({ message, type, onClose }) => (
  <div className={`toast toast-${type}`}>
    <span>{type === 'success' ? '✅' : type === 'error' ? '[X]' : '[i]'}</span>
    ...
    <div className="automation-rules">
      ...
      <div className="rules-section">
        <div className="rules-header">
          <h4>[Clipboard] Rules ({filteredRules.length}{filterType !== 'all' ? ' filtered' : ''})</h4>
          <div className="rules-header-actions">
            ...
            <button className="add-rule-btn" onClick={showForm ? cancelForm : openCreateForm}>
              {showForm && editingId === null ? 'X Cancel' : '+ Create Rule'}
            </button>
          </div>
        </div>
          ...
          <div className="log-list">
            {triggerLog.slice(0, 20).map(log => (
              <div key={log.id} className={`log-entry ${log.success ? '' : 'log-failed'}`}>
                <span className="log-status">{log.success ? '✅' : '[X]'}</span>
