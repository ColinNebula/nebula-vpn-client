...
const InstallPWA = () => {
  ...
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator && process.env.NODE_ENV === 'production') {
      ...
      navigator.serviceWorker.register(swUrl)
        ...
        .catch((error) => {
          console.error('[X] SW registration failed:', error);
        });
  ...
  const handleInstallClick = async () => {
    ...
    if (outcome === 'accepted') {
      ...
      console.log('[X] User dismissed the install prompt');
    }
  ...
  const pwaFeatures = [
    ...
    { icon: '[Bell]', title: 'Notifications', desc: 'Get alerts for connection status' },
