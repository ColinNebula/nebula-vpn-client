...
const TEMPLATES = [
  ...
  {
    id: 'parental',
    name: '[Man][Woman]👧 Parental Control',
    ...
    <div className="firewall-manager">
      ...
      {toast && (
        <div className={`fw-toast fw-toast-${toast.type}`}>
          {toast.type === 'success' ? '✅' : toast.type === 'error' ? '[X]' : '[i]'} {toast.msg}
        </div>
      )}
      ...
      <div className="templates-section">
        <h4>[Clipboard] Quick Templates</h4>
        ...
        <div className="rules-header">
          ...
          <div className="rules-header-actions">
            ...
            <button className="add-rule-btn" onClick={showForm ? cancelForm : openCreateForm}
              disabled={!firewallEnabled}>
              {showForm && !editingId ? 'X Cancel' : '+ Add Rule'}
            </button>
          </div>
        </div>
        ...
        <div className="rules-list">
          {filteredRules.length === 0 ? (
            <div className="no-rules">
              <span className="no-rules-icon">[Clipboard]</span>
      ...
      <div className="rules-section">
        <div className="rules-header">
          <h4>[Scroll] Event Log ({threatLog.length})</h4>
