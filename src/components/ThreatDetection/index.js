...
const THREAT_CATALOGUE = [
  ...
  { type: 'spyware',      icon: '[Detective]', color: '#4a148c', label: 'Spyware',        protocol: 'HTTPS', settingKey: 'blockMalware'      },
...
const ThreatDetection = ({ isConnected }) => {
  ...
  const getRiskLabel = () =>
    riskScore > 70 ? '🔴 HIGH' : riskScore > 40 ? '⚠️ MEDIUM' : '✅ LOW';
  ...
  return (
    <div className="threat-detection">
      ...
      <div className="view-tabs">
        {[
          { id: 'log',          label: `[Clipboard] Threat Log (${threats.length})` },
      ...
      {activeView === 'quarantine' && (
        <div className="view-panel">
          ...
          {quarantined.length === 0 ? (
            <div className="no-threats">
              <div className="no-threats-icon">[Unlocked]</div>
              ...
              {quarantined.map((domain, i) => (
                <div key={i} className="quarantine-item">
                  ...
                  <div className="quarantine-actions">
                    ...
                    <button onClick={() => removeFromQuarantine(domain)} className="q-btn release-btn">[Unlocked] Release</button>
                  </div>
                </div>
              ))}
            </div>
