      ...
      <div className={`md-hero ${activeConnection ? 'connected' : ''} ${isConnecting ? 'connecting' : ''}`}>
        ...
        <div className="md-hero-status">
          {isVPNPaused && isConnected
            ...
            : activeConnection
            ? <span className="status-chip paused">⚠️ Connected, Unverified</span>
            : <span className="status-chip off">⚠️ Unprotected</span>
          }
          {settings.protocol && (
      ...
      <div className="md-mid-row">
        ...
        <div className="md-card md-security-card">
          ...
          <p className="md-security-note">
            {securityScore >= 80 ? '✅ Excellent protection' : securityScore >= 50 ? '⚠️ Moderate - improve below' : '🔴 Low - verified tunnel recommended'}
          </p>
        ...
        <div className="md-card md-threats-card">
          ...
          <div className="md-threat-grid">
            {[
              ...
              { icon: '👁️', label: 'Trackers',   count: threatsBlocked.trackers, color: '#a78bfa' },
        ...
        <div className="md-card md-ip-card">
          ...
          {isSimulatedConnection && (
            <div style={{ background: '#7c2d12', color: '#fca5a5', borderRadius: '6px', padding: '8px 12px', marginBottom: '10px', fontSize: '12px', lineHeight: '1.4' }}>
              <strong>⚠️ Browser mode:</strong> No real VPN tunnel is active. Your actual IP and location are still visible to websites. Download the desktop app for real protection.
            </div>
          )}
          {isVerifiedConnection && (
          ...
          <div className="md-ip-row">
            ...
            <span className="ip-row-val">
              {isVerifiedConnection ? '🛡️ Nebula VPN Network' : '⚠️ Visible to websites'}
            </span>
          </div>
          <div className={`ip-status-banner ${isVerifiedConnection ? 'safe' : 'danger'}`}>
            {isVerifiedConnection ? '🔒 Handshake-Verified Tunnel' : isSimulatedConnection ? '🧪 Simulation Only - Identity Exposed' : '⚠️ Identity Exposed'}
          </div>
        </div>
      </div>
