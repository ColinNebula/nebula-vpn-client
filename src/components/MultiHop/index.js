...
const PRESETS = [
  ...
  { id: 'streaming',       name: 'Streaming',         icon: '▶️', desc: 'Two-hop optimised for low latency and bandwidth',   mode: 'standard', countries: ['US', 'GB'],        tags: ['netflix', 'bbc'],         color: '#E91E63' },
      ...
      <div className="mh-header">
        ...
        <div className="mh-header-actions">
          {selectedServers.length > 0 && !isConnected &&
            <button className="mh-btn-ghost" onClick={handleClearChain}>X Clear</button>}
          {!isConnected &&
                  ...
                  >
                    ...
                    {!isConnected && (
                      <div className="mh-hop-controls">
                        ...
                        <button className="mh-hop-ctrl remove" title="Remove"                                              onClick={() => handleRemoveServer(server.id)}>X</button>
                      </div>
                    )}
                  </div>
                </React.Fragment>
