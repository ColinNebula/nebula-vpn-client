...
// Check if we're running in Electron with GPS spoofing support
const hasGPSSupport = window.electron && window.electron.gps;
...
const GPSOverride = () => {
  ...
  const [systemGpsEnabled, setSystemGpsEnabled] = useState(false);
  ...
  // Fetch GPS status from Electron
  const checkGPSStatus = async () => {
    try {
      const status = await window.electron.gps.getStatus();
      setGpsStatus(status);
      setSystemGpsEnabled(status.enabled);
      if (status.location) {
        setSpoofedLocation({
          lat: status.location.lat,
          lng: status.location.lng,
          city: 'System GPS Location',
          accuracy: status.location.accuracy || 10,
        });
      }
    } catch (error) {
  ...
  // Toggle system-wide GPS spoofing
  const toggleSystemGPS = async () => {
    if (systemGpsEnabled) {
      // Disable GPS spoofing
      try {
        const result = await window.electron.gps.disable();
        if (result.success) {
          setSystemGpsEnabled(false);
          setSpoofedLocation(null);
          console.log('✅ System GPS spoofing disabled');
        } else {
          alert('Failed to disable GPS spoofing: ' + result.error);
        }
      } catch (error) {
        console.error('Failed to disable GPS spoofing:', error);
        alert('Failed to disable GPS spoofing: ' + error.message);
      }
    } else {
      // Enable GPS spoofing
      if (!spoofedLocation || (!spoofedLocation.lat && !spoofedLocation.lng)) {
      ...
      try {
        const result = await window.electron.gps.enable({
          location: {
            lat: spoofedLocation.lat,
            lng: spoofedLocation.lng,
            accuracy: spoofAccuracy === 'high' ? 10 : spoofAccuracy === 'medium' ? 50 : 100,
          },
          mode: 'registry', // Fixed: must be lowercase to match GPS_MODE.REGISTRY
        });

        if (result.success) {
          setSystemGpsEnabled(true);
          console.log('✅ System GPS spoofing enabled:', result);
        } else {
          alert('Failed to enable GPS spoofing: ' + result.error);
        }
      } catch (error) {
        console.error('Failed to enable GPS spoofing:', error);
        alert('Failed to enable GPS spoofing: ' + error.message);
      }
    }
  };
  ...
  return (
    <div className="gps-override">
      ...
      {/* Security notice - Updated to reflect system-wide support */}
      <div className="gps-notice" style={{ background: hasGPSSupport ? '#e8f5e9' : '#fff3cd', border: hasGPSSupport ? '1px solid #4caf50' : '1px solid #ffc107', borderRadius: '6px', padding: '10px 14px', marginBottom: '12px', fontSize: '13px', color: hasGPSSupport ? '#2e7d32' : '#856404' }}>
        {hasGPSSupport ? (
          <>
            <strong>✅ System-Wide GPS Spoofing Available!</strong> This Electron app can spoof GPS location at the OS level using Windows Location Service interception. Enable below to affect ALL applications on your system.
          </>
        ) : (
          <>
            <strong>⚠️ Notice:</strong> GPS Override stores your preferred location within this app only. For system-wide GPS spoofing, use the desktop Electron app with administrator privileges.
          </>
        )}
      </div>

      {/* System GPS Control (Electron only) */}
      {hasGPSSupport && (
        <div className="system-gps-control" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', borderRadius: '12px', padding: '16px', marginBottom: '20px', color: 'white' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h4 style={{ margin: '0 0 4px 0', fontSize: '1rem' }}>🌍 System-Wide GPS Spoofing</h4>
              <p style={{ margin: 0, fontSize: '0.85rem', opacity: 0.9 }}>
                {systemGpsEnabled ? 'All apps see spoofed location' : 'Enable to affect entire system'}
              </p>
            </div>
            <button 
              onClick={toggleSystemGPS}
              style={{
                padding: '10px 20px',
                background: systemGpsEnabled ? '#f44336' : 'white',
                color: systemGpsEnabled ? 'white' : '#667eea',
                border: 'none',
                borderRadius: '8px',
                fontWeight: '600',
                cursor: 'pointer',
            ...
            >
              {systemGpsEnabled ? '🛑 Disable' : '✅ Enable'}
            </button>
          </div>
          {systemGpsEnabled && gpsStatus && (
            <div style={{ marginTop: '12px', padding: '12px', background: 'rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '0.85rem' }}>
              <div>Mode: {gpsStatus.mode}</div>
              <div>Platform: {gpsStatus.platform}</div>
              {gpsStatus.location && (
                <div>Coordinates: {gpsStatus.location.lat.toFixed(4)}, {gpsStatus.location.lng.toFixed(4)}</div>
              )}
            </div>
          )}
        </div>
      ...
      <div className={`gps-status-panel ${gpsOverrideEnabled ? 'active' : ''}`}>
        <div className="status-row">
          ...
          <div className={`location-card spoofed ${gpsOverrideEnabled ? 'active' : ''}`}>
            <span className="location-label">[Privacy] Spoofed Location</span>
