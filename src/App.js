...
function App() {
  ...
  const isElectronApp = typeof window !== 'undefined' && (
    typeof window.electron?.vpn?.connect === 'function' ||
    typeof window.nebulaVPN?.vpn?.connect === 'function' ||
  ...
  const handleToggleConnection = async () => {
    ...
    const electronAPI = [window.electron, window.nebulaVPN]
      .find((api) => typeof api?.vpn?.connect === 'function') || null;
    ...
    console.log('🔍 Electron context detection:', {
      ...
      hasResolvedElectronAPI: !!electronAPI,
    ...
    // Never fall back to browser-mode API while running in Electron.
    // A missing preload bridge means local tunnel creation cannot work.
    if (!isElectronCtx && hasElectronUserAgent) {
      const bridgeError = 'Electron bridge unavailable. Restart Nebula VPN as Administrator and try again.';
      console.error('[X] Electron bridge missing - refusing browser fallback mode');
      setIsConnected(false);
      setProtectionState(buildProtectionState({ state: 'error', detail: bridgeError }));
      setIsConnecting(false);
      addLog(bridgeError, 'error');
      return;
    }
    ...
    if (isElectronCtx) {
      ...
      try {
        ...
        console.error('[X] VPN connection error:', err);
        setIsConnected(false);
    ...
    try {
      ...
      console.error('[X] API VPN connection error:', err);
      setIsConnected(false);
      setProtectionState(buildProtectionState({ state: 'error', detail: err.message }));
      setIsConnecting(false);
      addLog(`Connection error: ${err.message}`, 'error');
  ...
  return (
    <div className={`App ${isDarkMode ? 'dark-mode' : ''}`}>
      ...
      {/* Offline indicator */}
      {!isOnline && (
        <div className="offline-banner">⚠ No internet connection - Check your network
        </div>
      )}
      ...
      {killSwitchActive && (
        <div className="kill-switch-overlay">
          <div className="kill-switch-message">🛡 Kill Switch Active - All traffic blocked for security
          </div>
        </div>
      )}
      ...
      <header className="App-header">
        <div className="header-content">
          ...
          <div className="header-controls">
            ...
            {user?.role === 'admin' && (
              <button className="admin-dashboard-btn" onClick={() => setShowAdminDashboard(true)}>♛ Admin
              </button>
            )}
            <button className="donate-btn" onClick={() => setShowDonateModal(true)} title="Support Nebula VPN">♥ Donate
            </button>
            <button className="admin-btn" onClick={() => setShowAdminPanel(true)}>⚙ Settings
            </button>
      ...
      <div className="tab-nav-wrapper">
        <ResponsiveNavigation 
          ...
          onAdminToggle={() => setShowAdminDashboard(!showAdminDashboard)}
        />
      </div>
