...
class SecurityEnhancer {
  constructor(forceDevMode = false) {
  ...
  async enforceDNSThroughVPN(vpnAdapterName, trustedDNS) {
    ...
    // DEVELOPMENT MODE: Skip DNS enforcement entirely with multiple fallbacks for packaged apps
    const devChecks = {
      nodeEnv: process.env.NODE_ENV === 'development',
      forceDev: this.FORCE_DEV_MODE,
      configPath: vpnAdapterName?.includes('Local') || vpnAdapterName?.includes('AppData'),
    ...
    const isDevMode = devChecks.nodeEnv || devChecks.forceDev || devChecks.configPath || 
                      devChecks.isPackaged || devChecks.devBuild;
    
    console.log('[DNS Security] Development mode detection:', devChecks);
    console.log(`[DNS Security] Final development mode decision: ${isDevMode}`);
    
    if (isDevMode) {
      console.log('[DNS Security] 🔧 DEVELOPMENT MODE - SKIPPING DNS ENFORCEMENT (packaged app support)');
      console.log(`[DNS Security] 📝 Would have enforced DNS ${Array.isArray(trustedDNS) ? trustedDNS.join(',') : trustedDNS} on adapter: ${vpnAdapterName}`);
      return true; // Return success to continue dev mode simulation
    }

    console.log('[DNS Security] ⚡ PRODUCTION MODE - Enforcing real DNS leak prevention');
