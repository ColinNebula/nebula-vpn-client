...
const { GPSSpoofingManager, GPS_MODE } = require('./gps-spoofer');
...
console.log('[ID] MAIN.JS VERSION CHECK - FORCED SUCCESS VERSION LOADED:', new Date().toISOString());
...
async function isRunningAsAdmin() {
  if (process.platform === 'win32') {
    try {
      // Test write access to a system location - fastest and most reliable
      const testFile = 'C:\\Windows\\Temp\\nebula-admin-test.tmp';
      const fs = require('fs');
      fs.writeFileSync(testFile, '');
      fs.unlinkSync(testFile);
      console.log('🔐 Admin check: Can write to protected location - ADMIN');
      return true;
    } catch (error) {
      console.log('🔐 Admin check: Cannot write to protected location - NOT ADMIN');
      return false;
    }
  } else {
...
const gpsManager = new GPSSpoofingManager();
...
function getApiBase() {
  // Enhanced development mode detection for packaged apps
  ...
  if (isDevMode) {
    console.log('🔧 [getApiBase] Using localhost API (dev mode detected)');
    return 'http://localhost:3001/api';
  }
  
  console.log('🔧 [getApiBase] Using production API');
...
async function testApiConnectivity(apiBase) {
  console.log('[testApiConnectivity] Testing connectivity to:', apiBase);
  try {
    // For localhost, just try a simple HEAD request
    if (apiBase.includes('localhost')) {
      const testRes = await fetch(`${apiBase}/health`, { 
        method: 'HEAD',
        timeout: 5000 
      });
      console.log('[testApiConnectivity] Localhost test result:', testRes.status);
      return { reachable: true, status: testRes.status };
    } else {
      // For remote endpoints, test basic connectivity
      const testRes = await fetch(apiBase, { 
        method: 'HEAD',
        timeout: 10000 
      });
      console.log('[testApiConnectivity] Remote test result:', testRes.status);
      return { reachable: true, status: testRes.status };
    }
  } catch (error) {
    console.error('[testApiConnectivity] Connectivity test failed:', {
...
async function connectSingle({ serverId, protocol, token, killSwitch }) {
  ...
  try {
    ...
    console.log('[connectSingle] Using API endpoint:', apiBase);
    
    // Test API connectivity before proceeding
    console.log('[connectSingle] Testing API connectivity...');
    const connectivityTest = await testApiConnectivity(apiBase);
    if (!connectivityTest.reachable) {
      const errorMsg = connectivityTest.code === 'ECONNREFUSED' 
        ? `API server not running at ${apiBase}. Start your backend server first.`
        : `Cannot reach API server at ${apiBase}. Error: ${connectivityTest.error}`;
      throw new Error(errorMsg);
    }
    console.log('[connectSingle] ✓ API connectivity test passed');
    ...
    let res;
    try {
      console.log('[connectSingle] Making fetch request to:', `${apiBase}/vpn/connect`);
      res = await fetch(`${apiBase}/vpn/connect`, {
        method: 'POST',
        headers: {
      ...
      console.log('[connectSingle] Fetch response received. Status:', res.status, 'OK:', res.ok);
    } catch (fetchError) {
      console.error('[connectSingle] Fetch failed with detailed error:', {
      ...
      if (fetchError.code === 'ECONNREFUSED') {
        throw new Error(`Connection refused to ${apiBase}. Is the server running?`);
      } else if (fetchError.code === 'ENOTFOUND') {
        throw new Error(`Could not resolve hostname ${apiBase}. Check your internet connection and DNS settings.`);
      } else if (fetchError.code === 'ETIMEDOUT') {
        throw new Error(`Connection timed out to ${apiBase}. Server may be down or unreachable.`);
      } else {
        throw new Error(`Network error: ${fetchError.message} (${fetchError.code || 'unknown code'})`);
      }
    }
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // Use default session in packaged builds; a custom persistent partition
      // has caused renderer launch failures on some Windows installs.
      partition: isDev ? 'persist:nebula-vpn' : undefined,
  ...
  const productionBuildDir = path.join(app.getAppPath(), 'build');
  const productionBuildIndex = path.join(productionBuildDir, 'index.html');
  const productionTrustedPrefix = pathToFileURL(productionBuildDir + path.sep).href;

  // Security: block renderer from navigating to untrusted origins
  mainWindow.webContents.on('will-navigate', (event, url) => {
    const trusted = isDev
      ? url.startsWith('http://localhost:3000')
      : url.startsWith(productionTrustedPrefix);
    if (!trusted) {
  ...
  const startUrl = isDev
    ? 'http://localhost:3000'
    : pathToFileURL(productionBuildIndex).href;
  ...
  mainWindow.webContents.session.webRequest.onBeforeSendHeaders((details, callback) => {
    ...
    // Never mutate local file/app requests. In packaged builds the renderer
    // loads from file:// and changing headers here can cause ERR_FAILED.
    const isHttpLike = details.url.startsWith('http://') || details.url.startsWith('https://');
    if (!isHttpLike) {
  ...
  if (isDev || isPrivacyRegressionMode) {
    ...
    console.log('🔴 MAIN.JS - About to load file:', productionBuildIndex);
    mainWindow.loadFile(productionBuildIndex).catch((err) => {
      console.error('[X] MAIN.JS - loadFile failed:', err.message);
      // Fallback to URL form for additional diagnostics on edge installs.
      mainWindow.loadURL(effectiveStartUrl).catch((urlErr) => {
        console.error('[X] MAIN.JS - loadURL fallback failed:', urlErr.message);
      });
    });
  }
  ...
  mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    console.log('[X] MAIN.JS - Page failed to load!');
    console.log('[X] Error code:', errorCode);
    console.log('[X] Error description:', errorDescription);
    console.log('[X] URL:', validatedURL);
  ...
  mainWindow.webContents.on('dom-ready', () => {
    ...
    `).catch(err => {
      console.error('[X] [Privacy] Failed to inject privacy protections:', err);
    });
...
// ═══════════════════════════════════════════════════════════════════════════
// GPS SPOOFING IPC HANDLERS
// ═══════════════════════════════════════════════════════════════════════════

// Enable GPS spoofing with specified location
ipcMain.handle('gps-enable', async (event, { location, mode }) => {
  console.log('🌍 [IPC gps-enable] Handler called with:', { location, mode });
  try {
    const result = await gpsManager.enableSpoofing(location, mode);
    return result;
  } catch (err) {
    console.error('[IPC gps-enable] Error:', err.message);
    return { success: false, error: err.message };
  }
});

// Disable GPS spoofing
ipcMain.handle('gps-disable', async () => {
  console.log('🌍 [IPC gps-disable] Handler called');
  try {
    const result = await gpsManager.disableSpoofing();
    return result;
  } catch (err) {
    console.error('[IPC gps-disable] Error:', err.message);
    return { success: false, error: err.message };
  }
});

// Update GPS spoofed location
ipcMain.handle('gps-update', async (event, { location }) => {
  console.log('🌍 [IPC gps-update] Handler called with:', { location });
  try {
    const result = await gpsManager.updateLocation(location);
    return result;
  } catch (err) {
    console.error('[IPC gps-update] Error:', err.message);
    return { success: false, error: err.message };
  }
});

// Get GPS spoofing status
ipcMain.handle('gps-status', async () => {
  try {
    const status = gpsManager.getStatus();
    return { success: true, ...status };
  } catch (err) {
    console.error('[IPC gps-status] Error:', err.message);
    return { success: false, error: err.message };
  }
});

// Add GPS jitter for realistic movement simulation
ipcMain.handle('gps-add-jitter', async (event, { maxMeters }) => {
  try {
    const jitteredLocation = gpsManager.addGPSJitter(maxMeters || 5);
    if (jitteredLocation) {
      return { success: true, location: jitteredLocation };
    } else {
      return { success: false, error: 'GPS spoofing not enabled' };
    }
  } catch (err) {
    console.error('[IPC gps-add-jitter] Error:', err.message);
    return { success: false, error: err.message };
  }
});

// Get timezone for spoofed location
ipcMain.handle('gps-get-timezone', async (event, { location }) => {
  try {
    const timezone = gpsManager.getTimezoneForLocation(location);
    return { success: true, timezone };
  } catch (err) {
    console.error('[IPC gps-get-timezone] Error:', err.message);
    return { success: false, error: err.message };
  }
});
...
if (!isDev) {
  app.on('ready', () => {
    const configuredApiOrigin = (process.env.API_URL || '').replace(/\/api$/, '');
    const apiOrigins = [
      configuredApiOrigin,
      'https://api.nebula3ddev.com',
      'http://localhost:3001',
      ...
      callback({
        responseHeaders: {
          ...
          'Content-Security-Policy': [
            ...
            `connect-src 'self' file: ${apiOrigins} https://api6.ipify.org https://www.dnsleaktest.com https://*.dnsleaktest.com https://bash.ws https://*.bash.ws https://ipwho.is https://stun.cloudflare.com:3478; ` +
            `object-src 'none'; ` +
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    ipcMain.handle('vpn-connect', async (event, { serverId, protocol, token, killSwitch }) => {
      console.log('🔧 [IPC vpn-connect] Handler called with:', { serverId, protocol, hasToken: !!token, killSwitch });
      try {
        const result = await connectSingle({ serverId, protocol, token, killSwitch });
        if (result?.success) {
          lastConnectionState = {
            type: 'single',
    ...
    console.log('🎯 MAIN.JS - vpn-connect IPC handler registered successfully inside app.whenReady()');
    ...
    setTimeout(async () => {
      try {
        ...
        console.log('🔍 MAIN.JS - ipcMain listenerCount for vpn-connect:', ipcMain.listenerCount('vpn-connect'));
    ...
    console.error('🚨 MAIN.JS - Error registering vpn-connect handler:', err);
  }
