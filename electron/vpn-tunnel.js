...
class WireGuardTunnel {
  constructor() {
    ...
    this._savedPhysicalGateway = null; // physical gateway detected before WireGuard connects

    // PRODUCTION MODE: Real WireGuard tunnels with full internet connectivity
    // INDUSTRY STANDARD: Disabled development bypass for production deployment
    this.FORCE_DEV_MODE = false;  // PRODUCTION: Always use real WireGuard tunnels
    console.log('🔧 WireGuardTunnel constructor: FORCE_DEV_MODE =', this.FORCE_DEV_MODE, this.FORCE_DEV_MODE ? '(DEV MODE - BYPASSING REAL VPN)' : '(REAL VPN TUNNELS ENABLED)');
    ...
    // Initialize advanced security enhancer with development mode flag
    this._securityEnhancer = new SecurityEnhancer(this.FORCE_DEV_MODE);
  ...
  _resolveConfigDir() {
    switch (this.platform) {
      case 'win32':  
        // Use AppData for user-accessible configs (no admin required)
        return path.join(os.homedir(), 'AppData', 'Local', 'NebulaVPN', 'WireGuard');
  ...
  writeConfig({ privateKey, assignedIP, dns, serverPublicKey, serverEndpoint,
                allowedIPs = '0.0.0.0/0', presharedKey = null }) {
  ...
  // ── Gateway Detection (Critical for Split Tunneling) ──────────────────
  
  /**
   * Enhanced physical gateway detection with multiple fallback methods.
   * This MUST be called BEFORE WireGuard connects to prevent WiFi blocking.
   */
  async _detectPhysicalGateway() {
    const detectionMethods = [];
    let physicalGateway = null;

    // Method 1: PowerShell NetRoute (most reliable)
    try {
      const psCommand = `Get-NetRoute -DestinationPrefix "0.0.0.0/0" | Where-Object { 
      ...
      const { stdout } = await execAsync(`powershell.exe -NoProfile -Command "${psCommand}"`, { timeout: 5000 });
      const gateway = stdout.trim();
      
      if (gateway && gateway !== '0.0.0.0' && /^\d+\.\d+\.\d+\.\d+$/.test(gateway)) {
        physicalGateway = gateway;
        detectionMethods.push('PowerShell NetRoute');
      }
    } catch (err) {
      console.error('[Gateway] Method 1 failed:', err.message);
    }

    // Method 2: Route table parsing (fallback)
    if (!physicalGateway) {
      try {
        const { stdout } = await execAsync('route print 0.0.0.0', { timeout: 5000 });
        const lines = stdout.split('\n').filter(line => line.match(/^\s*0\.0\.0\.0\s+0\.0\.0\.0/));
        
        for (const line of lines) {
          const match = line.match(/(\d+\.\d+\.\d+\.\d+)\s+(\d+)\s*$/);
          if (match) {
            const gw = match[1];
            const metric = parseInt(match[2], 10);
            // Skip VPN gateways and prefer lower metrics
            if (gw !== '0.0.0.0' && (!physicalGateway || metric < 50)) {
              physicalGateway = gw;
              detectionMethods.push('Route Table');
              break;
            }
          }
        }
      } catch (err) {
        console.error('[Gateway] Method 2 failed:', err.message);
      }
    }

    // Method 3: WiFi adapter configuration (specific)
    if (!physicalGateway) {
      try {
        const psCommand = `Get-NetAdapter | Where-Object { 
          $_.Status -eq 'Up' -and ($_.Name -like '*WiFi*' -or $_.Name -like '*Wireless*' -or $_.InterfaceDescription -like '*Wireless*') 
        } | Select-Object -First 1 -ExpandProperty InterfaceIndex`;
        
        const { stdout } = await execAsync(`powershell.exe -NoProfile -Command "${psCommand}"`, { timeout: 3000 });
        const wifiIndex = stdout.trim();
        
        if (wifiIndex && /^\d+$/.test(wifiIndex)) {
          const gatewayCommand = `(Get-NetIPConfiguration -InterfaceIndex ${wifiIndex}).IPv4DefaultGateway.NextHop`;
          const { stdout: gwStdout } = await execAsync(`powershell.exe -NoProfile -Command "${gatewayCommand}"`, { timeout: 3000 });
          const gateway = gwStdout.trim();
          
          if (gateway && /^\d+\.\d+\.\d+\.\d+$/.test(gateway)) {
            physicalGateway = gateway;
            detectionMethods.push('WiFi Adapter');
          }
        }
      } catch (err) {
        console.error('[Gateway] Method 3 failed:', err.message);
      }
    }

    // Method 4: Common router IP ping test (last resort)
    if (!physicalGateway) {
      const commonGateways = ['192.168.1.1', '192.168.0.1', '10.0.0.1', '172.16.0.1'];
      
      for (const gw of commonGateways) {
        try {
          await execAsync(`ping -n 1 -w 2000 ${gw}`, { timeout: 3000 });
          physicalGateway = gw;
          detectionMethods.push('Ping Test');
          break;
        } catch (err) {
    ...
    if (physicalGateway) {
      console.log(`[Gateway] ✅ Detected: ${physicalGateway} (via: ${detectionMethods.join(', ')})`);
    } else {
      console.error('[Gateway] ❌ All detection methods failed!');
    }
  ...
  async _up() {
    // Development mode detection with multiple fallbacks for packaged apps
    const devChecks = {
      nodeEnv: process.env.NODE_ENV === 'development',
      forceDev: this.FORCE_DEV_MODE,
      configPath: this.configPath.includes('AppData\\Local\\NebulaVPN'),
      npmScript: process.env.npm_lifecycle_script?.includes('electron-dev'),
      // Additional detection for packaged development builds
      isPackaged: process.env.NODE_ENV !== 'production', // If not explicitly production, treat as dev
      userConfig: this.configPath.includes('AppData') || this.configPath.includes('Local'),
      devBuild: !process.resourcesPath || process.resourcesPath.includes('node_modules') // Detect if it's a dev build
    };
    
    // CRITICAL FIX: When FORCE_DEV_MODE = false, force real tunnels regardless of environment
    const isDevMode = this.FORCE_DEV_MODE === false ? false : (devChecks.nodeEnv || devChecks.forceDev || devChecks.configPath || 
                      devChecks.npmScript || devChecks.isPackaged || devChecks.userConfig || devChecks.devBuild);
                      
    console.log(`[WireGuard] Development mode detection:`, devChecks);
    console.log(`[WireGuard] Final development mode decision: ${isDevMode}`);

    if (isDevMode && this.platform === 'win32') {
      console.log('[WireGuard] 🔧 DEVELOPMENT MODE - simulating tunnel creation (packaged app support)');
      this._simulateDevTunnel();
      this.connected = true;
      return;
    }

    console.log('[WireGuard] ⚡ PRODUCTION MODE - Real WireGuard service installation');

    if (this.platform === 'win32') {
      // ===== CRITICAL: Detect physical gateway BEFORE WireGuard connects =====
      // This prevents the gateway detection failure that causes WiFi blocking
      console.log('[Routing] Detecting physical gateway BEFORE WireGuard connection...');
      this._savedPhysicalGateway = await this._detectPhysicalGateway();
      
      if (!this._savedPhysicalGateway) {
        console.warn('[Routing] WARNING: Could not detect physical gateway!');
        console.warn('[Routing] This may cause WiFi/LAN access issues when VPN connects.');
        console.warn('[Routing] Continuing anyway - you may need to run FIX-VPN-ROUTES-NOW.ps1');
      } else {
        console.log(`[Routing] ✅ Physical gateway saved: ${this._savedPhysicalGateway}`);
      }
      ...
      try {
        ...
        if (error.message.includes('Access is denied')) {
          throw new Error(
            'Administrator privileges required for WireGuard service installation.\n\n' +
            '🔧 SOLUTIONS:\n' +
            '1. Run as Administrator: Right-click start-vpn-admin.ps1 → "Run as Administrator"\n' +
            '2. Enable Dev Mode: Set NODE_ENV=development in environment variables\n' +
            '3. Use: npm run electron-dev (automatically sets dev mode)'
          );
        }
        throw error;
      }
      ...
      // Now uses the pre-detected gateway to prevent WiFi blocking
  ...
  async _ksWindows(action, serverIP) {
    // CRITICAL FIX: Add rule to allow ALL traffic through VPN interface
    // This prevents the kill switch from blocking legitimate VPN traffic
    const rules = [
      ...
      // This is the critical rule that was missing - allows all traffic through VPN tunnel
      { name: 'NebulaVPN-AllowVPNInterface', cmd: null }, // Handled separately with InterfaceAlias
    ];

    if (action === 'add') {
      // Add interface-specific rule first (most important for allowing VPN traffic)
      try {
        // Get the VPN adapter name
        const vpnAdapter = await this._getWindowsTunnelAdapter();
        
        if (vpnAdapter) {
          console.log(`[KillSwitch] Adding allow rule for VPN interface: ${vpnAdapter}`);
          
          // Use PowerShell for interface-based rule (netsh doesn't support -InterfaceAlias well)
          await execAsync(
            `powershell -NoProfile -Command "` +
            `if (-not (Get-NetFirewallRule -Name 'NebulaVPN-AllowVPNInterface' -ErrorAction SilentlyContinue)) { ` +
          ...
          console.log('[KillSwitch] ✅ VPN interface allow rule added (CRITICAL FIX)');
        } else {
          console.warn('[KillSwitch] Could not find VPN adapter for interface rule');
        }
      } catch (err) {
        console.error('[KillSwitch] Failed to add VPN interface rule:', err.message);
      }
      
      // Add other kill switch rules
      for (const { name, cmd } of rules) {
        if (cmd === null) continue; // Skip the interface rule (already added above)
      ...
      // Use an IPv6 CIDR rule for compatibility across Windows PowerShell versions.
      await execAsync(
        `powershell -NoProfile -NonInteractive -Command "if (-not (Get-NetFirewallRule -Name 'NebulaVPN-BlockIPv6' -ErrorAction SilentlyContinue)) { New-NetFirewallRule -Name 'NebulaVPN-BlockIPv6' -DisplayName 'Nebula VPN Block IPv6' -Direction Outbound -Action Block -RemoteAddress '::/0' -Enabled True | Out-Null }"`
      ).catch(e => console.warn('[KillSwitch] IPv6 block rule failed:', e.message));
    } else {
      // Remove all kill switch rules
      for (const { name } of rules) {
        if (name === 'NebulaVPN-AllowVPNInterface') {
          // Use PowerShell to remove interface rule
          await execAsync(
            `powershell -NoProfile -Command "Remove-NetFirewallRule -Name 'NebulaVPN-AllowVPNInterface' -ErrorAction SilentlyContinue"`
          ).catch(() => {});
        } else {
          await execAsync(
  ...
  async setDNS(dnsServers) {
    ...
    const devChecks = {
      ...
      userConfig: this.configPath.includes('AppData') || this.configPath.includes('Local'),
      devBuild: !process.resourcesPath || process.resourcesPath.includes('node_modules')
    };
    
    const isDevMode = devChecks.nodeEnv || devChecks.forceDev || devChecks.configPath || 
                      devChecks.npmScript || devChecks.isPackaged || devChecks.userConfig || devChecks.devBuild;
    
    console.log('[setDNS] Development mode detection:', devChecks);
    console.log(`[setDNS] Final development mode decision: ${isDevMode}`);
    
    if (isDevMode) {
      console.log('[setDNS] 🔧 DEVELOPMENT MODE - SKIPPING DNS ENFORCEMENT (packaged app support)');
      console.log(`[setDNS] 📝 Would have set DNS to: ${Array.isArray(dnsServers) ? dnsServers.join(', ') : dnsServers}`);
      // Track fake DNS servers for dev mode
      this._activeDnsServers = Array.isArray(dnsServers) ? [...dnsServers] : [dnsServers];
      return;
    }

    console.log('[setDNS] ⚡ PRODUCTION MODE - Applying real DNS enforcement');
  ...
  async restoreDNS() {
    ...
    if (isDevMode) {
      console.log('[restoreDNS] 🔧 DEVELOPMENT MODE - SKIPPING DNS RESTORATION (packaged app support)');
      this._activeDnsServers = [];
      return;
    }

    console.log('[restoreDNS] ⚡ PRODUCTION MODE - Restoring DNS settings');
  ...
  async _getWindowsTunnelAdapter() {
    ...
    const partial = adapters.find(adapter => {
      const lower = adapter.toLowerCase();
      return (
        lower.includes(this.tunnelName.toLowerCase()) ||
        lower.includes('wireguard') ||
        lower.includes('nebula vpn')
      );
    });
    if (partial) return partial;

    // Fallback: query all adapters (including disconnected) to find the WG tunnel.
    try {
      const psCmd =
        "Get-NetAdapter | Where-Object { " +
        "($_.InterfaceDescription -match 'WireGuard') -or " +
        "($_.Name -match 'Nebula VPN|Nebulavpn|WireGuard') " +
  ...
  async _setWindowsAdapterDescription() {
    ...
    try {
      ...
      // Keep WireGuard's adapter name stable so DNS/routing lookup continues to work.
      console.log(`[Adapter] Windows network adapter detected: ${adapter}`);
    } catch (error) {
  ...
  async _configureWindowsRouting() {
    try {
      ...
      if (interfaceIndex && /^\d+$/.test(interfaceIndex)) {
        console.log(`[Routing] Configuring routes for VPN tunnel (interface ${interfaceIndex})...`);
        
        // Use the pre-detected gateway (saved before WireGuard connected)
        let physicalGateway = this._savedPhysicalGateway;
        
        // Fallback: try current detection if pre-detection failed
        if (!physicalGateway) {
          console.warn('[Routing] No saved gateway found, attempting live detection...');
          const routeOutput = await execAsync('route print 0.0.0.0').catch(() => ({ stdout: '' }));
          const defaultRouteMatch = routeOutput.stdout.match(/0\.0\.0\.0\s+0\.0\.0\.0\s+(\d+\.\d+\.\d+\.\d+)/);
          physicalGateway = defaultRouteMatch ? defaultRouteMatch[1] : null;
        }
        
        if (physicalGateway && physicalGateway !== '0.0.0.0') {
          console.log(`[Routing] Using physical gateway: ${physicalGateway}`);
          
          // Add route for VPN server endpoint through physical gateway
          // This ensures WireGuard handshake packets can reach the server
          if (this._vpnServerIP && this._vpnServerIP !== '127.0.0.1') {
            const addServerRouteCmd = `route add ${this._vpnServerIP} mask 255.255.255.255 ${physicalGateway} metric 1`;
            await execAsync(addServerRouteCmd).catch(err => {
              // Route might already exist, that's okay
              console.log(`[Routing] Server route: ${err.message.includes('exists') ? 'already exists' : err.message}`);
            });
            console.log(`[Routing] Added route for VPN server ${this._vpnServerIP} via ${physicalGateway}`);
          }
          
          // ===== SPLIT TUNNEL: Preserve local network access =====
          // Add routes for private network ranges through physical gateway
          // This allows local WiFi/LAN access while routing internet through VPN
          console.log('[Routing] Configuring split tunnel for local network access...');
          ...
          for (const { network, mask, desc } of localNetworks) {
            const cmd = `route add ${network} mask ${mask} ${physicalGateway} metric 1`;
            await execAsync(cmd).catch(err => {
              if (!err.message.includes('exists')) {
                console.log(`[Routing] Local network ${desc} (${network}): ${err.message}`);
              }
            });
          }
          console.log('[Routing] ✅ Local networks will bypass VPN (WiFi access preserved)');
        } else {
          // Critical: No physical gateway detected!
          console.error('[Routing] ❌ CRITICAL: No physical gateway detected!');
          console.error('[Routing] This will likely cause WiFi/LAN access to be blocked when VPN routes are applied.');
          console.error('[Routing] User will need to run FIX-VPN-ROUTES-NOW.ps1 after connecting to restore access.');
          console.error('[Routing] Consider disconnecting VPN if network access is lost.');
          
          // We can still add VPN routes, but local network access will be broken
          console.warn('[Routing] Proceeding with VPN routes only (local network access may be lost)...');
        }
        
        // ===== FULL TUNNEL: Complete IP masking =====
        // Route ALL traffic through VPN for maximum privacy
        // Note: This may impact local network access
        
        console.log('[Routing] Implementing FULL tunneling for complete IP masking...');
        
        // Ensure critical local network access is preserved BEFORE full tunnel routes
        const localNetworks = [
          ...
          { network: '169.254.0.0', mask: '255.255.0.0', desc: 'Link-local addressing' },
          { network: '224.0.0.0', mask: '240.0.0.0', desc: 'Multicast' }
        ];
        ...
        console.log('[Routing] ✅ Local networks configured to bypass VPN');
        
        console.log(`[Routing] ✅ Full tunnel configured for complete privacy (interface ${interfaceIndex})`);
        console.log('[Routing] 🔒 All internet traffic will be routed through VPN');
        console.log('[Routing] ⚠️  Note: WireGuard will handle 0.0.0.0/0 routing automatically');
        
        // Verify DNS routes were added
        const verifyOutput = await execAsync('route print 1.1.1.1').catch(() => ({ stdout: '' }));
        const hasDnsRoutes = verifyOutput.stdout.includes(interfaceIndex);
        if (hasDnsRoutes) {
          console.log('[Routing] ✅ Route verification: DNS privacy routes are active');
        } else {
          console.warn('[Routing] ⚠️  Route verification: Could not confirm DNS routes');
        }
      } else {
        console.error('[Routing] Failed to get interface index');
      }
    } catch (error) {
      console.error('[Routing] ❌ Failed to configure default routing:', error.message);
      throw error; // Re-throw so connect() knows routing failed
    }
  }

  /**
   * Remove Windows VPN routes to restore normal routing.
   * Called during disconnect to clean up the routing table.
   */
  async _restoreWindowsRouting() {
    try {
      const adapter = await this._getWindowsTunnelAdapter();
      if (!adapter) {
        console.log('[Routing] No VPN adapter found, skipping route cleanup');
        return;
      }
      ...
      if (interfaceIndex && /^\d+$/.test(interfaceIndex)) {
        console.log(`[Routing] Removing VPN routes (interface ${interfaceIndex})...`);
        
        // Get current default gateway to remove local network routes
        const routeOutput = await execAsync('route print 0.0.0.0').catch(() => ({ stdout: '' }));
        const defaultRouteMatch = routeOutput.stdout.match(/0\.0\.0\.0\s+0\.0\.0\.0\s+(\d+\.\d+\.\d+\.\d+)/);
        const physicalGateway = defaultRouteMatch ? defaultRouteMatch[1] : null;
        
        // Remove local network routes (bypass routes cleanup)
        if (physicalGateway) {
          const localNetworks = ['192.168.0.0', '10.0.0.0', '172.16.0.0', '169.254.0.0', '224.0.0.0'];
          for (const network of localNetworks) {
            await execAsync(`route delete ${network}`).catch(err => {
              if (!err.message.includes('not found')) {
                console.log(`[Routing] Local network route cleanup (${network}): ${err.message}`);
              }
            });
          }
        }
        
        // Note: WireGuard automatically removes 0.0.0.0/0 routes when interface goes down
        console.log('[Routing] ✅ Full tunnel routes cleaned up automatically by WireGuard');
        
        // Remove VPN server route if it exists
        if (this._vpnServerIP && this._vpnServerIP !== '127.0.0.1') {
          await execAsync(`route delete ${this._vpnServerIP}`).catch(err => {
            if (!err.message.includes('not found')) {
              console.log(`[Routing] Server route cleanup: ${err.message}`);
            }
          });
        }
        
        console.log('[Routing] ✅ VPN routes removed, normal routing restored');
      }
    } catch (error) {
      console.warn('[Routing] Route cleanup error (non-fatal):', error.message);
    }
  }
  ...
  async _validateWindowsTrafficRouting(vpnIP) {
    ...
    try {
      ...
      // Look for DNS privacy routes instead of default routes (resilient split tunnel)
      const dnsRouteCheck = await execAsync('route print 1.1.1.1').catch(() => ({ stdout: '' }));
      const hasDnsRoutes = dnsRouteCheck.stdout.includes('10.8.0.');
      
      // Test DNS resolution through the tunnel
      let dnsPrivacyCheck = null;
      try {
        // Test if DNS queries are going through the VPN
        const { stdout: nslookupOut } = await execAsync(
          'nslookup google.com 1.1.1.1',
          { timeout: 5000 }
        );
        dnsPrivacyCheck = {
          status: nslookupOut.includes('Non-authoritative answer:') ? 'dns-working' : 'dns-issue',
          response: nslookupOut.trim()
        };
      } catch (e) {
        dnsPrivacyCheck = { status: 'dns-test-failed', error: e.message };
      }
      
      // Test internet connectivity (should work through normal routes)
      let internetCheck = null;
      try {
        const { stdout: httpOut } = await execAsync(
          'powershell.exe -NoProfile -Command "(Invoke-WebRequest -Uri "https://api.ipify.org" -UseBasicParsing -TimeoutSec 10).Content"',
          { timeout: 12000 }
        );
        const detectedIP = httpOut.trim();
        internetCheck = {
          detectedIP,
          status: 'internet-working',
          ipType: detectedIP.match(/^(192\.168\.|10\.0\.|172\.(1[6-9]|2[0-9]|3[01])\.)/) ? 'private' : 'public'
        };
      } catch (e) {
        internetCheck = { status: 'internet-failed', error: e.message };
      }
      
      console.log(`[Traffic Check] DNS privacy routes: ${hasDnsRoutes ? '✅ Active' : '[X] Not found'}`);
      console.log(`[Traffic Check] DNS resolution: ${dnsPrivacyCheck.status === 'dns-working' ? '✅' : '⚠️'} ${dnsPrivacyCheck.status}`);
      console.log(`[Traffic Check] Internet access: ${internetCheck.status === 'internet-working' ? '✅' : '❌'} ${internetCheck.status}`);
      
      return {
        hasDnsRoutes,
        dnsPrivacy: dnsPrivacyCheck,
        internetAccess: internetCheck,
        routingVerified: hasDnsRoutes && dnsPrivacyCheck.status === 'dns-working' && internetCheck.status === 'internet-working',
        status: hasDnsRoutes && internetCheck.status === 'internet-working' ? 'resilient-tunnel-active' : 'routing-issue'
      };
    } catch (error) {
  ...
  async _checkActiveDnsServersWindows() {
    try {
      ...
      console.log(`[DNS Check] Enforcement: ${matches ? '✅ VERIFIED' : '[X] MISMATCH'}`);
  ...
  async _dnsWindows(action, servers = []) {
    ...
    console.log('[_dnsWindows] Development mode detection:', devChecks);
    console.log(`[_dnsWindows] Final development mode decision: ${isDevMode}`);
    
    if (isDevMode) {
      console.log('[_dnsWindows] 🔧 DEVELOPMENT MODE - SKIPPING DNS ENFORCEMENT (packaged app support)');
      console.log(`[_dnsWindows] 📝 Would have set DNS to: ${Array.isArray(servers) ? servers.join(', ') : servers} on action: ${action}`);
      return; // Skip actual DNS changes in dev mode
    }

    console.log('[_dnsWindows] ⚡ PRODUCTION MODE - Applying real DNS enforcement');
    ...
    if (action === 'set') {
      ...
      try {
        ...
        if (error.message?.includes('elevation') || error.message?.includes('Administrator')) {
          ...
          console.error(`[DNS] [X] ${adminError.message}`);
          throw adminError;
        }
  ...
  * @param {number}         [params.dotPort=853]         – DoT upstream port
  ...
  async connect({
    ...
    try {
      ...
      this.writeConfig({
        ...
        allowedIPs:      '0.0.0.0/0', // Full tunneling: all traffic through VPN
        presharedKey,    // null → line omitted; non-null → PQC PSK applied
      });

      // Extract and save the original VPN server IP for routing configuration
      // This is the actual WireGuard server IP (not 127.0.0.1 if using obfuscation)
      this._vpnServerIP = serverEndpoint.split(':')[0];
      console.log(`[WireGuard] VPN server IP: ${this._vpnServerIP}`);
  ...
  async disconnect() {
    ...
    // Restore normal routing before DNS cleanup
    if (this.platform === 'win32') {
      await this._restoreWindowsRouting().catch(e =>
        console.error('[WireGuard] Route cleanup error:', e.message));
    }
  ...
  async verifyTunnelIntegrity() {
    ...
    try {
      ...
      console.log(`[Tunnel Check] Handshake: ${results.handshake.verified ? '✅' : '[X]'}`);

      // 2. Verify DNS enforcement (Windows only)  
      if (this.platform === 'win32') {
        results.dns = await this._checkActiveDnsServersWindows();
        console.log(`[Tunnel Check] DNS: ${results.dns.enforcementVerified ? '✅' : '[X]'}`);
      }
      ...
      if (this.platform === 'win32') {
        ...
        console.log(`[Tunnel Check] Routing: ${results.routing.status === 'verified' ? '✅' : '[X]'}`);
      }

      // 4. Verify IPv6 leak prevention
      results.ipv6 = await this.verifyIPv6LeakPrevention();
      console.log(`[Tunnel Check] IPv6: ${results.ipv6.status === 'verified' ? '✅' : '[X]'}`);
      ...
      console.log(`[Tunnel Check] Overall verification: ${results.verified ? '✅ PASSED' : '[X] FAILED'}`);
      return results;
...
class SplitTunnelManager {
  ...
  async _getGateway() {
    ...
    if (this.platform === 'win32') {
      ...
      if (m) { this._gateway = { ip: m[1], iface: m[2] }; return this._gateway; }
    } else {
