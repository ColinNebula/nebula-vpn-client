...
try {
    ...
    Write-Host "  ❌ Route analysis failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "  ❌ Route priority analysis failed" -ForegroundColor Red
}
...
if (-not $pingWorking -and -not $externalIPWorking) {
    ...
    Write-Host "Run this command as Administrator:" -ForegroundColor Yellow
    Write-Host ".\WORKING-VPN-CLEANUP.ps1" -ForegroundColor White
} else {
    Write-Host "Run this command as Administrator:" -ForegroundColor Yellow
    Write-Host ".\WORKING-VPN-CLEANUP.ps1" -ForegroundColor White
}
